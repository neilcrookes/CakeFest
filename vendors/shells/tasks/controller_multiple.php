...
class ControllerMultipleTask extends ControllerTask {
  ...
  function bakeViewAction() {
    extract($this->_actionVars);
		...
		$actions .= "\t\tif (!\$id) {\n";
		if ($wannaUseSession) {
			$actions .= "\t\t\t\$this->Session->setFlash(__('Invalid {$singularHumanName}.', true), '{$admin}flash_bad');\n";
  ...
  function bakeAddAction() {
    ...
    $actions = "\n";
		...
		if ($wannaUseSession) {
			$actions .= "\t\t\t\t\$this->Session->setFlash(__('The ".$singularHumanName." has been saved', true), '{$admin}flash_good');\n";
      ...
      $actions .= "\t\t\t\t}\n";
		...
		$actions .= "\t\t\t} else {\n";
		if ($wannaUseSession) {
			$actions .= "\t\t\t\t\$this->Session->setFlash(__('The {$singularHumanName} could not be saved. Please, try again.', true), '{$admin}flash_bad');\n";
		}
		$actions .= "\t\t\t}\n";
		$actions .= "\t\t}\n";
  ...
  function bakeDeleteAction() {
    extract($this->_actionVars);
		...
		$actions .= "\t\tif (!\$id) {\n";
		if ($wannaUseSession) {
			$actions .= "\t\t\t\$this->Session->setFlash(__('Invalid id for {$singularHumanName}', true), '{$admin}flash_bad');\n";
			$actions .= "\t\t\t\$this->redirect(array('action'=>'index'));\n";
		} else {
			$actions .= "\t\t\t\$this->flash(__('Invalid {$singularHumanName}', true), array('action'=>'index'));\n";
		}
		...
		if ($wannaUseSession) {
			$actions .= "\t\t\t\$this->Session->setFlash(__('{$singularHumanName} deleted', true), '{$admin}flash_good');\n";
			$actions .= "\t\t\t\$this->redirect(array('action'=>'index'));\n";
		} else {
